...
accT_graph.add_trace(
    go.Scattergl(
        x =  time_test,
        y =  difference_out,#np.log10(difference_out),
        #xaxis_type = "log",
        mode='lines',
        name='Reference',
        marker_color = 'green'
...
accT_graph.update_layout(xaxis_type="log", yaxis_type="log")
# line_1 = np.array()
...
fusion_graph.update_layout(
    # title="Fourier transform of Payload A and Reference Accelerometer",
    # title_x=0.5,
    xaxis_title="Frequency (Hz)",
    yaxis_title="Gain",
)

fusion_graph.update_layout(legend=dict(
    orientation="h",
    yanchor="bottom",
    y=1.02,
    xanchor="right",
...
psd_graph.add_trace(
    go.Scattergl(
        x =  time_pieton,
        y =  vib_pieton_filtered,#[153000:180000:],vib_pieton,#
        mode='markers',
        name='Vibration pieton',
        marker_color = '#cc810d' #aa99ff #eca537 #cc810d
    ),
)
...
gain_graph.update_layout(
    title="Frequency responses of 2 Payloads",
    title_x=0.5,
    xaxis_title="Frequency (Hz)",
    yaxis_title="Gain",
)
...
gain_graph.add_trace(
    go.Scattergl(
        ...
        y =  abs(Pxx_ratio)[:],
        mode='lines',
        name='Payload A : Reference (less volume of shock absorber)'
    ),
)
gain_graph.add_trace(
    go.Scattergl(
        ...
        mode='lines',
        name='Payload B : Reference (more volume of shock absorber)'
    ),
)
...
# Set y-axes titles
arduino_graph.update_yaxes(title_text="Temperature (Celsius)", secondary_y=False)
arduino_graph.update_yaxes(title_text="Humidity (mg) and Lighting (Lux)", secondary_y=True)
...
adc_speeds = []
for i in range(24531, len(adc_flight01[:-1])):
    #print(adc_flight01[i+1])
    diff = adc_flight01[i+1]-adc_flight01[i]
    duration = x_adc[i+1]-x_adc[i]
    adc_speeds.append(diff/duration)
...
for i in range(len(arduino_time[:-1])):
    #print(adc_flight01[i+1])
    diff = arduino_temp[i+1]-arduino_temp[i]
    duration = arduino_time[i+1]-arduino_time[i]
    temp_speeds.append(diff/duration)
